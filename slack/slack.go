...
import (
	...
	"gitlab.sicepat.tech/platform/golib/env"
)
...
func (s *slackImpl) PostToSlack(ctx context.Context, message string, opts ...PostOption) error {
	...
	result := api.Post(s.webhookURL).WithContext(ctx).WithTimeout(s.timeout).
		WithBody(buf).Execute()

	if result.Error != nil {
		return fmt.Errorf("failed to send webhook request: %w", result.Error)
	}
	...
	return fmt.Errorf("failed to send slack message, status: %d body: %s", result.Response.StatusCode,
